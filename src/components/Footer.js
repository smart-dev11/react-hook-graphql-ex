...
export default function Footer()  {
  ...
  const randomColour = useCallback(() => '#'+(Math.random()*0xFFFFFF<<0).toString(16), [])

  const renderDelta = useMemo(() => {
    return (
      <div style={{background: randomColour()}}> Delta is {delta} </div>
    )
  }, [delta])

  const renderNum = useMemo(() => {
    return (
      <div style={{background: randomColour()}}> Counter is {num} </div>
    )
  }, [num])
